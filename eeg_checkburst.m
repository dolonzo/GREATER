...
if nargin < 2
    geometry = {[1, 0.5]};
    uilist = {...
        {'Style', 'text', 'string', 'Event marking TMS pulses'}...
        {'Style', 'edit', 'string', '' 'tag' 'event'} };
    [~, ~, ~, outstruct, ~] = inputgui('geometry', geometry, 'uilist', uilist, 'title', 'Check for burst pattern stimulation--eeg_checkburst()');
    [EEG, isburst] = eeg_checkburst(EEG, split(outstruct.event));
